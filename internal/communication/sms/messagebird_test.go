...
func TestNewMessageBirdProvider(t *testing.T) {
	provider := NewMessageBirdProvider("test-api-key")
	assert.NotNil(t, provider)
	assert.NotNil(t, provider.client)
}

func TestMessageBirdProvider_SendSMS(t *testing.T) {
	tests := []struct {
		name    string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr {
				...
				// With invalid API key, API call will fail
				assert.Error(t, err)
			}
		})
	}
...
func TestMessageBirdProvider_SendBulkSMS(t *testing.T) {
	provider := NewMessageBirdProvider("test-api-key")
	ctx := context.Background()
