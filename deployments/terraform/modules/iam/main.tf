# IAM Module - Service Roles and Policies for Gorax
# Creates IAM roles for EKS service accounts using IRSA

data "aws_caller_identity" "current" {}

# API Service Role (for Secrets Manager, SQS, S3 access)
    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "${var.oidc_provider}:sub" = "system:serviceaccount:${var.namespace}:gorax-api"
...
# Worker Service Role (for SQS, S3, KMS access)
resource "aws_iam_role" "worker_service" {
  name = "${var.name_prefix}-worker-service-role"
    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "${var.oidc_provider}:sub" = "system:serviceaccount:${var.namespace}:gorax-worker"
            "${var.oidc_provider}:aud" = "sts.amazonaws.com"
          }
        }
      }
...
resource "aws_iam_role_policy" "worker_service" {
  name = "${var.name_prefix}-worker-service-policy"
...
resource "aws_iam_role_policy" "eks_node_cloudwatch" {
  name = "${var.name_prefix}-eks-node-cloudwatch-policy"
